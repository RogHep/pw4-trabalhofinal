...
export default function EditarImovel() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setTipos(tipoList); setBairros(bairroList);
        const im = await buscarImovel(id);
        // adaptar preço BigDecimal se vier como string
        setForm({
          ...im,
  ...
  function handleChange(e) {
    ...
    if (name === "tipoImovel" || name === "bairro") {
      setForm(f => ({ ...f, [name]: { id: value ? Number(value) : null } }));
  ...
  return (
    <div>
      ...
      <form onSubmit={onSubmit} className="bg-white p-6 rounded shadow space-y-4">
        <div>
          <label>Título</label>
          <input name="titulo" value={form.titulo || ""} onChange={handleChange} className="mt-1 block w-full rounded border-gray-300 p-2" />
        </div>
        {/* Repita os campos conforme CriarImovel — para brevidade deixei apenas alguns */}
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => navigate(-1)} className="px-4 py-2 rounded bg-gray-200">Cancelar</button>
          <button type="submit" className="px-6 py-2 rounded bg-indigo-600 text-white">Salvar</button>
        </div>
      </form>
    </div>
  );
